...
## 2026-08-31（同日稍晚）：Model Card 與儀表板揭露展示樣本限制

**問題**：留一攻擊實驗做完後，各項發現（資料集限制、各驗證分數可信度、樣本量、留一攻擊結果、目前不具備的能力）分散在多份文件裡，沒有一份單頁摘要可以直接回答「這個模型能做什麼、不能做什麼」。另外，儀表板雖然在頂部說明是離線重播，但沒有明確講清楚 97 筆展示樣本是刻意平衡挑選的，畫面上的攻擊比例、漏報比例可能被誤讀成真實比例。

**做了什麼**：
- 新增 `docs/model_card.md`：一頁式整理模型能力邊界、資料集限制、各驗證分數的可信程度對照表、各攻擊類型樣本量與可靠度分級、留一攻擊實驗結論、目前不具備的能力、建議優先順序。
- `web/index.html` 加上一段常駐提示：展示樣本經平衡挑選、本次重播統計不代表真實比例，並指向完整測試集的正式評估結果與 Model Card。

**驗證**：瀏覽器確認提示正確顯示在畫面上、無 console／伺服器錯誤；22/22 測試通過。

**相關檔案**：`docs/model_card.md`、`web/index.html`
